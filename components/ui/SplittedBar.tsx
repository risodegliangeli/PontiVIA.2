...
export default function CustomTabBar(props: any) {
  ...
  const onHolydaysPress = () => {
    if (isHolydaysFocused) return; // ESCE SE GIA' SELEZIONATO
  ...
  const onIndexPress = () => {
    if (isIndexFocused) return; // ESCE SE GIA' SELEZIONATO
  ...
  const onPreferencesPress = () => {
    if (isPreferencesFocused) return; // ESCE SE GIA' SELEZIONATO
  ...
  const RailBeyond = () => {
    ...
    useEffect(() => {
      ...
      Animated.timing(animatedValue, {
        toValue: targetValue,
        duration: 500,
        easing: Easing.elastic(1),        // easing standard per evitare salti
    ...
    const scaleX = animatedValue.interpolate({ // VALORE ORIZZONTALE
      inputRange: [-1, -0.8, -0.2, 0, 0.2, 0.8, 1],
      outputRange: [.8, 1.5, 1.5, .75, 1.5, 1.5, 1], // plateau di deformazione
    });
    // PARAMETRI ALLUNGAMENTO VERTICALE
    const scaleY = animatedValue.interpolate({ // VALORE VERTICALE
      inputRange: [-1, -0.8, -0.2, 0, 0.2, 0.8, 1],
      outputRange: [.8, 0.6, 0.6, .75, 0.6, 0.6, 1], // plateau di deformazione
    });
